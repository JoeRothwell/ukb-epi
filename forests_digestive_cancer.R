...
library(readxl)
library(tidyverse)
t2 <- read_xlsx("forest_data_metS.xlsx", sheet = 1)
ly <- read_xlsx("forest_data_metS.xlsx", sheet = 2)
...
forest(x = t2$estimate, ci.ub = t2$ci.high, ci.lb = t2$ci.low, 
       ...
       refline=1, efac = 0, rows = rowvec, ylim=c(0,34), 
...
forest(x = t2$estimate, ci.ub = t2$ci.high, ci.lb = t2$ci.low, 
       ...
       refline=1, efac = 0, rows = rowvec, ylim=c(0,34), xlab="HR (95% CI)", psize= 1.5, cex=1.5, 
