...
t2 <- read_xlsx("forest_data_metS.xlsx", sheet = 3) %>% filter(category != "Diabetes")
ly <- read_xlsx("forest_data_metS.xlsx", sheet = 4) %>% slice(1:22)
...
rowvec1 <- rev(which(rev(ly$rowvec1)))
library(metafor)
par(mar=c(5,0,0,1))
forest(x = t2$estimate, ci.ub = t2$ci.high, ci.lb = t2$ci.low, 
       subset = t2$subsite == "oesophad_inc" & t2$analysis == "normal",
       refline=1, efac = c(0, 0.5), rows = rowvec1, ylim=c(0, limm + 3), 
       xlab="HR (95% CI)", 
       header = c("adenocarcinoma", ""), annosym = c(" (", ", ", ")"),
       psize= 1.5, cex=1.5, slab = NA, pch = 18, xlim = c(0.8, 5.5), #alim = c(0, 3),
       cex.lab = 1)
text(0.5, limm + 3, "Oesophageal", pos = 4, font = 2, cex = 1.5)
...
forest(x = t2$estimate, ci.ub = t2$ci.high, ci.lb = t2$ci.low, 
       ...
       refline = 1, efac = c(0, 0.5), rows = rowvec2, ylim=c(0, limm + 3), 
       xlab="HR (95% CI)", psize= 1.5, cex=1.5, annosym = c(" (", ", ", ")"),
...
forest(x = t2$estimate, ci.ub = t2$ci.high, ci.lb = t2$ci.low, 
       ...
       refline=1, efac = c(0, 0.5), rows = rowvec2, ylim=c(0, limm + 3), xlab="HR (95% CI)", 
...
forest(x = t2$estimate, ci.ub = t2$ci.high, ci.lb = t2$ci.low, 
       ...
       annosym = c(" (", ", ", ")"),
       psize= 1.5, cex=1.5, slab = NA, pch = 18, xlim = c(0.8, 6), #alim = c(0,4),
       cex.lab = 1)
